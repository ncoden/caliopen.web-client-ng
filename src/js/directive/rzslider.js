...
// * remove visibility manipulations
// * move ticks to index 2 in template
// * remove handle half size from selBar position calc
// * remove labels
// * remove options: getSelectionBarColor
// * remove single handle feature
// * change bindings: model and high become min and max in one-way
...
(function(root, factory) {
  ...
  .factory('RzSlider', function($timeout, $document, $window, $compile, RzSliderOptions, rzThrottle) {
    ...
    var Slider = function(scope, sliderElem) {
      ...
      /**
       * The slider inner low value (linked to rzSliderMin)
       * @type {number}
       */
      this.lowValue = 0;

      /**
       * The slider inner high value (linked to rzSliderMax)
       * @type {number}
       */
      this.highValue = 0;
       ...
       */
      this.range = this.scope.rzSliderMin !== undefined && this.scope.rzSliderMax !== undefined;
    ...
    Slider.prototype = {
      ...
      init: function() {
        ...
        this.scope.$watch('rzSliderMin', function(newValue, oldValue) {
        ...
        this.scope.$watch('rzSliderMax', function(newValue, oldValue) {
          if (self.internalChange)
            return;
          if (newValue === oldValue)
            return;
      ...
      syncLowValue: function() {
        if (this.options.stepsArray) {
          if (!this.options.bindIndexForStepsArray)
            this.lowValue = this.findStepIndex(this.scope.rzSliderMin);
          else
            this.lowValue = this.scope.rzSliderMin
        }
        else
          this.lowValue = this.scope.rzSliderMin;
      },
      ...
      syncHighValue: function() {
        if (this.options.stepsArray) {
          if (!this.options.bindIndexForStepsArray)
            this.highValue = this.findStepIndex(this.scope.rzSliderMax);
          else
            this.highValue = this.scope.rzSliderMax
        }
        else
          this.highValue = this.scope.rzSliderMax;
      },
      ...
      applyLowValue: function() {
        if (this.options.stepsArray) {
          if (!this.options.bindIndexForStepsArray)
            this.scope.rzSliderMin = this.getStepValue(this.lowValue);
          else
            this.scope.rzSliderMin = this.lowValue
        }
        else
          this.scope.rzSliderMin = this.lowValue;
      },
      ...
      applyHighValue: function() {
        if (this.options.stepsArray) {
          if (!this.options.bindIndexForStepsArray)
            this.scope.rzSliderMax = this.getStepValue(this.highValue);
          else
            this.scope.rzSliderMax = this.highValue
        }
        else
          this.scope.rzSliderMax = this.highValue;
      },
      ...
      updateAriaAttributes: function() {
        this.minH.attr({
          'aria-valuenow': this.scope.rzSliderMin,
          'aria-valuetext': this.customTrFn(this.scope.rzSliderMin, this.options.id, 'model'),
          'aria-valuemin': this.minValue,
          'aria-valuemax': this.maxValue
        });
        if (this.range) {
          this.maxH.attr({
            'aria-valuenow': this.scope.rzSliderMax,
            'aria-valuetext': this.customTrFn(this.scope.rzSliderMax, this.options.id, 'high'),
            'aria-valuemin': this.minValue,
            'aria-valuemax': this.maxValue
          });
        }
      ...
      updateSelectionBar: function() {
        var position = 0,
          ...
          positionForRange = this.options.rightToLeft ? this.maxH.rzsp : this.minH.rzsp;
      ...
      getEventPosition: function(event) {
        ...
        if (this.options.vertical)
          ...
          eventPos = this.getEventXY(event) - sliderPos;
        return eventPos * this.options.scale;
      },
      ...
      callOnStart: function() {
        if (this.options.onStart) {
          ...
          this.scope.$evalAsync(function() {
            self.options.onStart(self.options.id, self.scope.rzSliderMin, self.scope.rzSliderMax, pointerType);
          });
        }
      },
      ...
      callOnChange: function() {
        if (this.options.onChange) {
          var self = this,
            pointerType = this.tracking === 'lowValue' ? 'min' : 'max';
          this.scope.$evalAsync(function() {
            self.options.onChange(self.options.id, self.scope.rzSliderMin, self.scope.rzSliderMax, pointerType);
          });
        }
      },
      ...
      callOnEnd: function() {
        if (this.options.onEnd) {
          var self = this,
            pointerType = this.tracking === 'lowValue' ? 'min' : 'max';
          this.scope.$evalAsync(function() {
            self.options.onEnd(self.options.id, self.scope.rzSliderMin, self.scope.rzSliderMax, pointerType);
          });
        }
   ...
   * @name ngScope
   *
   * @property {number} rzSliderMin
   * @property {number} rzSliderMax
